...
## Dasar Teori
Multi List adalah struktur data yang terdiri dari kumpulan beberapa list berbeda yang saling terintegrasi satu sama lain melalui keterhubungan tertentu. Dalam struktur ini, setiap elemen di dalam list utama memiliki kemampuan untuk membentuk atau mengepalai list-nya sendiri. Secara umum, sistem ini menerapkan hierarki yang membedakan antara list induk (sebagai daftar utama) dan list anak (sebagai daftar turunan yang menempel pada elemen induk tertentu).
...
```
Program ini dirancang untuk mengonstruksi struktur data Multi Linked List yang menyediakan fungsionalitas manajemen data yang kompleks dan terintegrasi. Fitur-fitur utamanya mencakup pembaruan informasi (update), mekanisme pencarian (searching) yang mendalam, serta prosedur penghapusan (delete) elemen. Secara spesifik, program ini memiliki kemampuan untuk melacak hubungan hierarkis melalui fungsi pencarian elemen induk (parent) dan elemen turunan (child). Selain itu, sistem ini mendukung proses penambahan data secara simultan, di mana pengguna dapat memasukkan data induk beserta data anak yang terkait langsung di bawahnya.
...
### Kesimpulan
Materi ini membahas penerapan Multi Linked List dan Circular Linked List sebagai bentuk struktur data yang bersifat dinamis. Multi Linked List diimplementasikan untuk menggambarkan relasi hierarkis antara elemen induk (parent) dan elemen turunan (child). Sementara itu, Circular Linked List dikembangkan untuk menciptakan struktur data melingkar di mana elemen terakhir terhubung kembali ke elemen awal. Kedua model ini memfasilitasi operasi fundamental seperti penambahan, penghapusan, pencarian, serta navigasi data secara sistematis dan optimal.
